...
import sevenLogo from '@/assets/7-eleven_logo.svg';
import familyMartLogo from '@/assets/familymart-icon.svg';
import martIcon from '@/assets/the-mart-icon.svg';
export function useMap({
  ...
  function setMarkers(martList: MartDataType[]) {
    ...
    const familyMartIcon = L.icon({
      iconUrl: store === '7-11' ? sevenLogo : familyMartLogo, // 根據條件動態選擇圖片
      iconSize: [iconSize.value, iconSize.value],
    })
  
    const theMartIcon = L.icon({
      iconUrl: martIcon, // 固定圖標圖片
